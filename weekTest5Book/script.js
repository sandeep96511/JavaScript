...
async function fetchBooks(text) {
  let data = await fetch(
    `https://books-backend.p.goit.global/books/category?category=${text}`
  );
  let res = await data.json();
  createObject(res);
}
...
function createObject(res) {
  let bookArray = res.map((item) => {
    return {
      ...
      links: [
        item.buy_links[0].url,
        item.buy_links[1].url,
        item.buy_links[2].url,
      ],
...
document.getElementById("redirectButton").addEventListener("click", redirectFunction);
